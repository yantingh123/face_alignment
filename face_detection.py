...
if __name__ == '__main__':
    imgName = r'F:\codes\FaceSR_dp\232194_1.jpg'
    imageArray = cv2.imread(imgName)
    Image.fromarray(imageArray)
    face_landmarks_list = face_recognition.face_landmarks(imageArray, model="large")
    face_landmarks_dict = face_landmarks_list[0]
    print(face_landmarks_dict, end=" ")

    visualize_landmark(image_array=imageArray, landmarks=face_landmarks_dict)
    plt.show()

    aliFace, eyeCenter, theta = align_face(image_array=imageArray, landmarks=face_landmarks_dict)
    Image.fromarray(np.hstack((imageArray, aliFace)))
    visualize_landmark(image_array=aliFace, landmarks=face_landmarks_dict)
    plt.show()

    r_landmarks = rotate_landmarks(landmarks=face_landmarks_dict,
                                   eye_center=eyeCenter, angle=theta, row=imageArray.shape[0])
    visualize_landmark(image_array=aliFace, landmarks=r_landmarks)
    plt.show()

    cropped_face, leftX, topY = corp_face(image_array=aliFace, landmarks=r_landmarks)
    Image.fromarray(cropped_face)

    trans_landmarks = transfer_landmark(landmarks=r_landmarks, left=leftX, top=topY)
    visualize_landmark(image_array=cropped_face, landmarks=trans_landmarks)
    plt.show()
